...
export default function ProjectsPage() {
  ...
  const handleVolumeOfTasksChange = (lang: "uz" | "oz" | "ru", value: string) => {
    const newVolumeOfTasks = value
    if (lang === "uz") {
      setUz((prev) => ({ ...prev, volume_of_tasks: newVolumeOfTasks }))
    } else if (lang === "oz") {
      setOz((prev) => ({ ...prev, volume_of_tasks: newVolumeOfTasks }))
    } else {
      setRu((prev) => ({ ...prev, volume_of_tasks: newVolumeOfTasks }))
    }
  }

  const handleCustomerChange = (lang: "uz" | "oz" | "ru", value: string) => {
    const newCustomer = value
    if (lang === "uz") {
      setUz((prev) => ({ ...prev, customer: newCustomer }))
    } else if (lang === "oz") {
      setOz((prev) => ({ ...prev, customer: newCustomer }))
    } else {
      setRu((prev) => ({ ...prev, customer: newCustomer }))
    }
  }

  const handleStatusChange = (lang: "uz" | "oz" | "ru", value: string) => {
    const newStatus = value
    if (lang === "uz") {
      setUz((prev) => ({ ...prev, status: newStatus }))
    } else if (lang === "oz") {
      setOz((prev) => ({ ...prev, status: newStatus }))
    } else {
      setRu((prev) => ({ ...prev, status: newStatus }))
    }
  }

  const handleImplementationPeriodChange = (lang: "uz" | "oz" | "ru", value: string) => {
    const newImplementationPeriod = value
    if (lang === "uz") {
      setUz((prev) => ({ ...prev, implementation_period: newImplementationPeriod }))
    } else if (lang === "oz") {
      setOz((prev) => ({ ...prev, implementation_period: newImplementationPeriod }))
    } else {
