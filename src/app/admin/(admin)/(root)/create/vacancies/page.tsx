...
export default function VacanciesPage() {
  ...
  const addArrayItem = (lang: "en" | "uz" | "ru", field: "conditions" | "requirements" | "responsibilities") => {
    const updatedLang = { ...eval(lang) }
  ...
  const handlePublish = async () => {
    setIsLoading(true)
    try {
      const res = await axios.post("/api/vacancies", { en, uz, ru, salary, slug, category })
      if (res) {
        toast({
          title: "Success",
    ...
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        <Input
          ...
          onChange={
            lang === "en"
              ? handleEnTitleChange
              : (e) => eval(`set${lang.charAt(0).toUpperCase() + lang.slice(1)}({ ...${lang}, title: e.target.value })`)
          }
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          onChange={(e) =>
            eval(`set${lang.charAt(0).toUpperCase() + lang.slice(1)}({ ...${lang}, company: e.target.value })`)
          }
        />
      </div>
      {["conditions", "requirements", "responsibilities"]?.map((field) => (
        <div key={field} className="space-y-2">
          ...
          <Button type="button" variant="outline" onClick={() => addArrayItem(lang, field as "conditions" | "requirements" | "responsibilities")}>
            Add {field.slice(0, -1)}
          </Button>
        </div>
      ))}
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex gap-4">
            <Button className={cn("flex-1", isLoading && "animate-pulse")} onClick={handlePublish} disabled={isLoading}>Publish</Button>
          </div>
        </CardContent>
      </Card>
